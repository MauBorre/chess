...
#https://github.com/mgold/Python-snippets/blob/master/pygame_angles.py
"""
Angles in Pygame
...
while True:
    ...
    '''Necesito invertir esta operacion?
    Lo que yo busco es saltar siempre en un mismo angulo
    Mover fichas de un ajedrez en la correcta perspectiva'''
    #Angle logic
    dx = pos[0] - origin[0]
    dy = pos[1] - origin[1]
    rads = atan2(-dy,dx)
    ...
    '''esta es la recta que marca el angulo, pero no es precisamente lo que necesitamos
    creo...'''
    pygame.draw.line(screen, purple, origin, pos, 2) #<<  recta 'angular'   <<
    ...
    screen.blit(font.render(str(degs)[:5]+" degrees", True, green), (10, 510))
    screen.blit(font.render(str(rads)[:5]+" radians", True, green), (200, 510))
